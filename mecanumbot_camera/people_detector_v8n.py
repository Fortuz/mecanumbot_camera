...
try:
    ...
    ORT_IMPORT_ERROR = repr(e)
    print("[people_detector] onnxruntime import error:", ORT_IMPORT_ERROR, file=sys.stderr)
    traceback.print_exc()
    ort = None
...
class PeopleDetector(Node):
    def __init__(self):
        ...
        if ort is None:
            ...
            if ORT_IMPORT_ERROR:
                self.get_logger().warn(f"onnxruntime import error detail: {ORT_IMPORT_ERROR}")
    ...
    def on_image(self, img_msg: Image):
        ...
        try:
            ...
            t1 = time.time()
            self.get_logger().info(f"YOLO inference: {(t1 - t0)*1000:.1f} ms")
        except Exception as e:
