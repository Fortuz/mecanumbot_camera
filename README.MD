...
flowchart LR
    ...
    behavior -->|/cmd_accessory_pos| grip
...
flowchart TB
    ...
    A[Camera Node - /camera/image_raw]

    B1[Ball Detector - color/shape]
    B2[Blob detection]
    B3[Output: /detections/ball]

    C1[Person Detector - preprocess]
    C2[TFLite inference]
    C3[Output: /detections/person]
...
The Node Architecture Diagram shows the ROS2 node-level architecture,
while the second diagram gives a simplified view of the data flow through the system.
...
behavior_manager
  ...
  -> /cmd_accessory_pos  (std_msgs/Float64)
...
- **Outputs:** `/cmd_vel`, `/cmd_accessory_pos`.
...
**Behavior parameter reference**

| Parameter | Purpose | Notes |
|-----------|---------|-------|
| `fetch_enter_px` | Ball bbox width threshold to leave TRACK and enter FETCH | Larger values make the robot start the approach earlier. |
| `fetch_stop_px` | Ball bbox width to switch from FETCH to GRASP | Ensures the robot is close enough before stopping. |
| `REQUIRED_BALL_STABLE` | Number of consecutive frames required to consider the ball "stable" | Prevents flicker-driven transitions. |
| `REQUIRED_PERSON_STABLE` | Stable person frames before entering DELIVER | Avoids false positives. |
| `ball_lost_timeout` | Seconds without detections before considering the ball lost | Triggers a fallback to SEARCH. |
| `person_lost_timeout` | Seconds without detections before rotating again | Keeps FIND_OWNER responsive. |
| `min_find_owner_time` | Minimum dwell time in FIND_OWNER | Stops immediate transitions to DELIVER. |
| `new_ball_absent_time` | Time the ball must vanish after delivery before SEARCH restarts | Prevents cycling on the same ball. |
| `Kp_rot` / `Kp_fwd` | Proportional gains for angular/linear control | Adjust heading correction and forward speed. |
| `max_ang` / `max_lin` | Saturation limits for motion commands | Protects the drivetrain. |
| `search_speed` | Angular velocity while scanning | Controls how fast the robot spins in SEARCH. |
| `owner_threshold_px` | Person bbox width required for delivery | Sets the standoff distance to the owner. |

Most behavior tuning values are ROS 2 parameters and can be changed while the node is running:

```bash
ros2 param list /behavior_manager
ros2 param get /behavior_manager fetch_enter_px
ros2 param get /behavior_manager fetch_stop_px
ros2 param set /behavior_manager fetch_enter_px 150
ros2 param set /behavior_manager fetch_stop_px 200
ros2 param set /behavior_manager max_lin 0.15
```

### Behavior Manager State Machine

The robot's behavior is controlled by a six-state FSM:
...
### SEQUENCE DIAGRAM: What happens when the robot sees a ball?
```mermaid
sequenceDiagram
    autonumber

    Camera->>BallDetector: image_raw
    BallDetector->>BehaviorManager: ball detection
    BehaviorManager->>RobotBase: cmd_vel (rotate or approach)
    
    RobotBase-->>BehaviorManager: position feedback (implicit)

    BallDetector->>BehaviorManager: ball_width increases
    BehaviorManager->>Gripper: close (start grasp)
    
    BehaviorManager->>PersonDetector: request person position
    PersonDetector->>BehaviorManager: person detection
    
    BehaviorManager->>RobotBase: approach owner
    BehaviorManager->>Gripper: open (deliver)
...
Additional `ball_tracker_rgb` knobs (median blur, morphology, circularity, etc.) are declared in the node itself and can be changed via `ros2 param set` or by editing the defaults in `ball_tracker_rgb.py`. `behavior_manager` tuning values are also declared as ROS 2 parameters, so they can be changed live with `ros2 param set /behavior_manager <name> <value>`.
...
|----------|---------|------|
| `fetch_enter_px` | Ball bbox width threshold to enter FETCH | Determines when the robot begins slow approach |
| `fetch_stop_px` | Ball bbox width to switch to GRASP | Marks when robot is close enough to grasp |
| `REQUIRED_BALL_STABLE` | Number of stable frames required | Prevents flickering detections |
| `REQUIRED_PERSON_STABLE` | Stable person frames before delivery | Ensures the robot approaches a real target |
| `ball_lost_timeout` | Seconds without detection -> ball lost | Used for fallback to SEARCH |
| `person_lost_timeout` | Seconds without detection -> rotate again | Prevents random drifting |
| `min_find_owner_time` | Minimum time spent searching owner | Avoids accidental quick transitions |
| `Kp_rot` | Rotation control gain | Affects turning speed |
| `Kp_fwd` | Forward speed gain | Affects how aggressively the robot approaches |
| `max_ang` | Max angular velocity | Safety limit |
| `max_lin` | Max linear velocity | Safety limit |
| `search_speed` | Angular speed during SEARCH | Controls scanning |
| `owner_threshold_px` | Person bbox width required for delivery | Defines distance threshold |
